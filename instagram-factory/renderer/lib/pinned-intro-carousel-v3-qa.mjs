export async function runPinnedIntroCarouselV3Qa(page, { project, item, expectedAssetUrl }) {
  return page.evaluate(({ expectedText, expectedHeadlineIndexes, safeZone, expectedVisualCoreCount, usesApprovedPhoto, expectedAssetUrl }) => {
    ...
    const approvedImages = images.filter((image) => image.dataset.approvedAsset === 'MAHTZduqLyI');
    const photoSourceExact = approvedImages.every((image) => image.src === expectedAssetUrl && image.naturalWidth === 1152 && image.naturalHeight === 1536);
    ...
    const photoPanel = document.querySelector('.photo-copy-panel');
    const photoPanelRect = photoPanel ? rect(photoPanel.getBoundingClientRect()) : null;
    ...
    const checks = {
      ...
      approvedPhotoFullFrame: usesApprovedPhoto ? photoPresentation.every(({ rect: imageRect, objectFit, objectPosition }) => (
        imageRect.left === 0 && imageRect.top === 0 && imageRect.width === 1080 && imageRect.height === 1440
        && objectFit === 'contain' && objectPosition === '50% 50%'
      )) : true,
      photoTextClearsFigures: usesApprovedPhoto ? photoPanelRect && photoPanelRect.bottom <= 440 : true,
      ...
      noForcedCta: !/(подпиш|сохран|перешли|поделись|переходи|напиши)/iu.test(allText),
