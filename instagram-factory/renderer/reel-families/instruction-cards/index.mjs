...
function hookScene(scene) {
  return sceneShell(scene, 'card-hook', `
    ...
    ${contentNode(scene.text[0], 'hook-title display-serif', accentHtml(scene.text[0], scene.accent), animated('.1', '.5', 'clip'))}
...
function stepScene(scene, index) {
  return sceneShell(scene, `card-step card-step-${index % 2 ? 'odd' : 'even'}`, `
    ...
    ${contentNode(scene.text[1], 'step-action display-serif', accentHtml(scene.text[1], scene.accent), animated('.62', '.5', 'up'))}
...
function phraseScene(scene) {
  return sceneShell(scene, 'card-phrase', `
    ...
    <div class="phrase-plate" data-color data-motion-item data-at=".4" data-duration=".5" data-from="scale">
      ${contentNode(scene.text[1], 'phrase-quote display-serif', accentHtml(scene.text[1], scene.accent), animated('.85', '.6', 'clip'))}
    </div>
...
function ctaScene(scene, reel, { photoAssetUrl }) {
  ...
  return sceneShell(scene, `card-cta${reel.photoAsset ? ' card-cta-photo' : ''}`, `
    ...
    ${contentNode(scene.text[1], 'cta-title display-serif', accentHtml(scene.text[1], scene.accent), animated('.5', '.55', 'clip'))}
    ${photo}`);
}
